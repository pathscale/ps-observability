//! The socket transport, connecting half: finding a running host, and talking
//! to it.
//!
//! # Why this is a sibling of the server and not a layer above it
//!
//! A peer can be both. AgencyZero is driven by agents over this protocol while
//! it drives a browser it embeds, and a QA harness that launches a headless
//! host is a client of one socket in a process that could serve another. The
//! two halves share the vocabulary, the framing and the error taxonomy, and
//! neither is built on the other, so a build takes whichever halves it needs.
//!
//! Neither half needs blitz. The framing is the protocol; the document is not.
//! `ps-qa` takes this feature and nothing else, and `cargo tree -p ps-qa`
//! showing no renderer, window runtime or GPU stack is the check that keeps it
//! that way.
//!
...
//! second one that built its requests as untyped JSON by hand, with a comment
//! explaining which fields the runtime would and would not accept.
...
fn read_descriptor(path: &Path) -> Result<Descriptor> {
    ...
    if descriptor.protocol_version != DEBUG_PROTOCOL_VERSION {
        bail!(
            "descriptor {} uses debug protocol {}, but this client speaks {}",
            path.display(),
...
mod tests {
    ...
    fn descriptor_protocol_mismatch_is_rejected_before_connecting() {
        ...
        assert!(
            error.to_string().contains("this client speaks"),
